...
Ray Camera::generateRay(int x, int y) {
    ...
    Vector3f randomPoint = this->upperLeft + this->pixelDeltaU * next_float() + this->pixelDeltaV * next_float();
    randomPoint = randomPoint + x * this->pixelDeltaU + y * this->pixelDeltaV;

    direction += Normalize(randomPoint - this->from);
