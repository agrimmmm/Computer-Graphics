    ...
    if (node.primCount != 0) {
        ...
        for (uint32_t i = 0; i < node.primCount; i++) {
            ...
            if (siIntermediate.t <= ray.t && siIntermediate.didIntersect) {
                ...
                si.a = si.n;
                // float min = std::abs(si.a.x);
                // if(min > std::abs(si.a.y))
                //     min = std::abs(si.a.y);
                // if(min > std::abs(si.a.z))
                //     min = std::abs(si.a.z);

                // if(min == std::abs(si.a.x))
                //     si.b = Normalize(Vector3f(0, -si.a.z, si.a.y));
                // else if(min == std::abs(si.a.y))
                //     si.b = Normalize(Vector3f(-si.a.z, 0, si.a.x));
                // else if(min == std::abs(si.a.z))
                //     si.b = Normalize(Vector3f(-si.a.y, si.a.x, 0));

                si.b = Normalize(Cross(si.a, -ray.d));

                si.c = Normalize(Cross(si.a, si.b));
