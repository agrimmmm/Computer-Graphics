...
void Scene::parse(std::string sceneDirectory, nlohmann::json sceneConfig)
{
    ...
    //Lights
    Light getlights;
    this->Lights = getlights.load(sceneConfig);
        ...
        for (uint32_t i = 0; i < node.primCount; i++) {
            ...
            if (siIntermediate.t <= ray.t) {
                ...
                if(this->surfaces[this->getIdx(i + node.firstPrim)].hasDiffuseTexture())
                    si.tex = this->surfaces[this->getIdx(i + node.firstPrim)].diffuseTexture;
                else
                {
                    si.tex.resolution = Vector2i(0, 0);
                    si.diffuse = this->surfaces[this->getIdx(i + node.firstPrim)].diffuse;
                }
            }
        }
    }
