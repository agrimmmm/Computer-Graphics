...
Vector3f Texture::nearestNeighbourFetch(Vector2f uv, Vector2i res)
{
    float tempx = uv.x * res.x;
    float tempy = uv.y * res.y;
    ...
    int xreq = xbottom, yreq = ybottom;
    if(tempx - (float)xbottom > (float)xtop - tempx)
        xreq = xtop;
    if(tempy - (float)ybottom > (float)ytop - tempy)
        yreq = ytop;

    // std::cout << xreq << " " << yreq << std::endl;

    return loadPixelColor(xreq, yreq);
}

Vector3f Texture::bilinearFetch(Vector2f uv, Vector2i res)
{
    float tempx = uv.x * res.x;
    float tempy = uv.y * res.y;
    ...
    Vector3f c1 = loadPixelColor(xbottom, ybottom);
    Vector3f c2 = loadPixelColor(xtop, ybottom);
    Vector3f c3 = loadPixelColor(xbottom, ytop);
    Vector3f c4 = loadPixelColor(xtop, ytop);

    Vector3f cu = c1*((float)xtop - tempx) + c2*(tempx - (float)xbottom);
    Vector3f cl = c3*((float)xtop - tempx) + c4*(tempx - (float)xbottom);

    Vector3f color = cu*(ytop - (float)tempy) + cl*((float)tempy - ybottom);
    return color;
}
