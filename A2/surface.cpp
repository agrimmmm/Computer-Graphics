...
float Surface::TriangleArea(Vector3f v1, Vector3f v2, Vector3f v3)
{
    float a = Cross((v3 - v1), (v2 - v1)).Length();
    if(a < 0)
        a = -a;
    return a/2;
}
    ...
    if (node.primCount != 0) {
        ...
        for (uint32_t i = 0; i < node.primCount; i++) {
            ...
            if (siIntermediate.t <= ray.t && siIntermediate.didIntersect) {
                ...
                float area = TriangleArea(this->tris[this->getIdx(i + node.firstPrim)].v1, this->tris[this->getIdx(i + node.firstPrim)].v2, this->tris[this->getIdx(i + node.firstPrim)].v3);
                float alpha = TriangleArea(this->tris[this->getIdx(i + node.firstPrim)].v2, this->tris[this->getIdx(i + node.firstPrim)].v3, si.p)/area;
                float beta = TriangleArea(this->tris[this->getIdx(i + node.firstPrim)].v1, this->tris[this->getIdx(i + node.firstPrim)].v3, si.p)/area;
                float gama = TriangleArea(this->tris[this->getIdx(i + node.firstPrim)].v1, this->tris[this->getIdx(i + node.firstPrim)].v2, si.p)/area;
                ...
                si.uv = uv1*alpha + uv2*beta + uv3*gama;
            }
        }
    }
    else {
